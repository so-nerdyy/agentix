...
```powershell
$env:AGENTIX_PACKAGE = "@so-nerdyy/agentix@2.1.2"
irm https://raw.githubusercontent.com/so-nerdyy/agentix/main/install.ps1 | iex
```

```sh
AGENTIX_PACKAGE=@so-nerdyy/agentix@2.1.2 curl -fsSL https://raw.githubusercontent.com/so-nerdyy/agentix/main/install.sh | sh
```
...
$env:AGENTIX_PACKAGE='.release/so-nerdyy-agentix-2.1.2.tgz'
...
```bash
npm run release:manifest
AGENTIX_PACKAGE=.release/so-nerdyy-agentix-2.1.2.tgz \
