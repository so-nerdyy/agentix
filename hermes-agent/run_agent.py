...
if os.environ.get("AGENTIX_FRONTEND") == "hermes":
    try:
        ...
        AIAgent = AgentixBackend  # type: ignore[assignment]
        logger.info(
            "AIAgent rebind: using AgentixBackend (AGENTIX_FRONTEND=hermes)"
        )
    except Exception as exc:
        logger.warning("AgentixBackend rebind failed: %s", exc)
